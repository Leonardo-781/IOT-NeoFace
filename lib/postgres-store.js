...
function createPostgresStore(config = {}) {
  ...
  async function insertTelemetry(record, source = 'mqtt') {
    ...
    const bme280 = sensors.bme280 || {};
    const ldr = sensors.ldr || {};

    await query(
      `INSERT INTO telemetry (
        ts, gateway_id, node_id, device_id, seq, rssi, battery, payload,
        temperature_c, humidity_pct, pressure_hpa, light_pct, source
      ...
      [
        ...
        bme280.temperature_c ?? null,
        bme280.humidity_pct ?? null,
        bme280.pressure_hpa ?? null,
        ldr.light_pct ?? null,
        source
      ]
    );
